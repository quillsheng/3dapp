...
$(document).ready(function () {
  ...
  xmlHttp.onreadystatechange = function () {
    if (xmlHttp.readyState == 4) {
      ...
      response = JSON.parse(xmlHttp.response).data;//.split("~");
      // Loop round the response array
      for (var i = 0; i < response.length; i++) {
        // Handeler to build the HTML string
        // Use this to provide a link to the image
        htmlCode += '<a href="/static/assets/images/gallery_images/' + response[i] + ' " ' + ' data-fancybox data-caption="My X3D model render" > ';
        htmlCode += '<img class="card-img-top img-thumbnail" src="/static/assets/images/gallery_images/' + response[i] + '"/>';
        htmlCode += '</a>';
      }
